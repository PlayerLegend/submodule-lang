...
inline static void lang_tree_stack_push_ref (window_lang_tree_node_pp * stack, lang_tree_node ** node)
{
    if (*node)
    {
	*window_push (*stack) = node;
...
inline static lang_tree_node ** lang_tree_stack_pop_ref (window_lang_tree_node_pp * stack)
{
    if (range_is_empty(stack->region))
    {
	return NULL;
